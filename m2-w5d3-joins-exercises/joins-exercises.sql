...
SELECT customer.first_name, customer.last_name, COUNT(rental.customer_id) AS rental_number
FROM rental 
...
-- address.address
SELECT store.store_id, address.address, COUNT(rental.rental_id) AS number_of_rentals, SUM(payment.amount) AS total_sales, AVG(payment.amount) AS average_sale
FROM store
JOIN address ON store.address_id = address.address_id
